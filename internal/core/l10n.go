...
var Table = map[string][4]string{
	...
	"menu.autoStart":     {"Start at login", "开机自启", "ログイン時に起動", "로그인 시 자동 실행"},
	...
	"settings.autoStart": {"Start at login", "开机自启", "ログイン時に起動", "로그인 시 자동 실행"},
	...
	"about.origin": {"A rewrite of wdkwdkwdk/tinyfire (macOS, MIT) in Go with go-gui, for Windows, Linux and macOS.",
		"本项目是 wdkwdkwdk/tinyfire（macOS，MIT 协议）的 Go 重写版，运行于 Windows、Linux 与 macOS。",
		"wdkwdkwdk/tinyfire（macOS, MIT）を Go + go-gui で再実装し、Windows・Linux・macOS に対応。",
		"wdkwdkwdk/tinyfire(macOS, MIT)를 Go + go-gui로 재작성해 Windows, Linux, macOS에서 실행됩니다."},
	"about.powered": {"Go 1.26+ with go-gui · no installer · nothing else to install",
		"Go 1.26+ 与 go-gui · 免安装 · 无需额外运行时",
		"Go 1.26+ と go-gui · インストーラ不要 · 追加ランタイム不要",
		"Go 1.26+ 및 go-gui · 설치 불필요 · 런타임 추가 설치 불필요"},
