...
const (
	...
	// heartbeatInterval is the fallback scan period. It can be relaxed once the file
...
func (m *UsageMonitor) Start() {
	...
	// Headless callers must wait for the delayed baseline too.
	m.scanning.Store(true)
	go m.scheduleScans()
}

// scheduleScans owns both timers, so Stop cannot race with an AfterFunc
// callback installing a replacement timer. File notifications remain immediate.
func (m *UsageMonitor) scheduleScans() {
	baseline := time.NewTimer(250 * time.Millisecond)
	defer baseline.Stop()
