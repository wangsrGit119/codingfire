...
func (s *UsageStore) loadEventsLocked() {
	...
	for _, line := range safeReadLines(path) {
		if line == "" {
			continue
		}
		total++
...
func (s *UsageStore) RecentEvents(since time.Time, limit int) []core.UsageEvent {
	...
	out := make([]core.UsageEvent, 0, len(s.events))
	for _, e := range s.events {
...
// safeReadLines reads a file line by line, tolerating the fact that the writer
// (Claude Code, Codex, ...) still holds it open.
