...
// memImageBudget caps the in-memory image registry.
//
// Every frame is registered under a fresh key, because gui.UseImage has no
// invalidation API: re-registering an existing key with the same dimensions is
// a no-op that leaves the stored buffer alone, so the only way to hand the
// renderer new pixels is a new key. The backend's GPU texture cache is a
// 128-entry LRU, so past that it is one create plus one destroy per frame.
//
// That churn is the documented reason the solar_system example samples on the
// CPU instead — but that warning is about nine textures per frame. One campfire
// panel is a single ~66 KB upload per frame, which is nothing, so the trade is
// taken deliberately here. The budget is sized to roughly match the GPU cache
// (128 frames) rather than the 32 MiB default, which would hold ~500 frames.
...
type App struct {
	...
	fireMu sync.Mutex
	// frameSeq monotonically numbers rendered frames; it is the mem-image key.
	frameSeq uint64
...
func (a *App) Run() {
	...
	a.gw = gui.NewWindow(gui.WindowCfg{
		State:       a,
	...
	a.hover = gui.NewWindow(gui.WindowCfg{
...
func (a *App) tickLoop() {
	...
	for {
		...
		case <-t.C:
			a.fireMu.Lock()
			a.Fire.Tick()
			phase := a.Fire.LiveSnapshot.Phase
			a.fireMu.Unlock()
			a.pollDrag()
			...
			a.updateHover()
			frameTick++
			// The state remains 20Hz, but full RGBA composition is expensive.
			// Active flames refresh at ~10Hz; an idle/transparent flame only
			// needs ~2Hz for fade-out and hover positioning.
			frameEvery := 10
			if phase == core.PhaseFlame || phase == core.PhaseEmber {
				frameEvery = 2
			}
			if w := a.gw; w != nil && frameTick%frameEvery == 0 {
				...
				// and wakes the backend every 50ms.
				w.InvalidateLayout()
			}
...
func (a *App) flameView(w *gui.Window) gui.View {
	...
	a.Renderer.Render(snap, a.Settings.Size.PixelScale(), dragging, elapsed)

	a.mu.Lock()
	a.frameSeq++
	key := "codingfire/frame/" + strconv.FormatUint(a.frameSeq, 10)
	pw, ph := a.panelPhysW, a.panelPhysH
	lw, lh := a.panelLogW, a.panelLogH
	a.mu.Unlock()

	// The buffer is registered under a fresh key each frame — see
	// memImageBudget for why, and why that is acceptable here.
	src := gui.UseImage(key, pw, ph, a.Renderer.Pix())
	if src == "" {
...
// Hover tuning.
//
// hoverMissFrames and hoverRefreshMs are the C# build's _hoverMissCount = 6 and
// CardRefreshMs = 120. The miss count exists because the flame moves: a
// pixel-exact hit test drops out for a frame now and then, and without the grace
// period the card would strobe.
const (
	hoverMissFrames = 6
	hoverRefreshMs  = 120
	hoverGap        = 8
