...
type WindowCfg struct {
	// BitmapFrame presents a CPU-rendered bitmap through UpdateLayeredWindow
	// on Windows, without creating an OpenGL context or a text system. Requires
	// Transparent and DecorationNone. Called on the GUI thread; nil skips a
	// presentation. RGBA (premultiplied) and NRGBA buffers are supported directly.
	// Other platforms ignore it and use the window's ordinary view.
	BitmapFrame func(*Window) image.Image
	// Software renders normal views through the Windows bitmap backend.
	// Other platforms use their default backend. No OpenGL context is created.
	Software bool
	// InitiallyHidden suppresses the initial ShowWindow on Windows. The caller
	// can show the native window after positioning it or preparing its contents.
