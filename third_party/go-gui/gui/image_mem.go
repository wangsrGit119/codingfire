...
// UpdateImage is the explicit mutable alternative for GL/software animations.
//
...
func UseImage(key string, w, h int, pix []byte) string {
	return useImage(key, w, h, pix, false)
}
...
func UpdateImage(key string, w, h int, pix []byte) string {
	return useImage(key, w, h, pix, true)
}

func useImage(key string, w, h int, pix []byte, dynamic bool) string {
	...
	if mi, ok := memImages.data[key]; ok {
		// Static images keep their original pixels. Dynamic images overwrite
		// the owned buffer so the backend can refresh the existing texture.
		if mi.w == w && mi.h == h && mi.dynamic == dynamic {
			if dynamic {
				copy(mi.pix, pix)
			}
	...
	memImages.data[key] = &memImage{
		src: src, pix: pix, elem: elem, w: w, h: h, dynamic: dynamic,
	}
...
func LookupImage(src string) (w, h int, pix []byte, ok bool) {
	return lookupImage(src, false)
}

// LookupDynamicImage resolves only UpdateImage buffers. The GL backend checks
// these before its immutable texture cache. The same GUI-thread rule applies.
func LookupDynamicImage(src string) (w, h int, pix []byte, ok bool) {
	return lookupImage(src, true)
}

func lookupImage(src string, dynamicOnly bool) (w, h int, pix []byte, ok bool) {
	if !isMemImage(src) {
	...
	if !found || (dynamicOnly && !mi.dynamic) {
		return 0, 0, nil, false
	}
	memImages.order.MoveToBack(mi.elem)
