...
// updateTexture uploads new pixels without allocating another GPU texture.
// The caller guarantees unchanged dimensions and runs on the GL thread.
func updateTexture(tex glTexture, pixels []byte) {
	gogl.BindTexture(gogl.TEXTURE_2D, tex.id)
	gogl.TexSubImage2D(gogl.TEXTURE_2D, 0, 0, 0, tex.w, tex.h,
		gogl.RGBA, gogl.UNSIGNED_BYTE, unsafe.Pointer(&pixels[0]))
	gogl.BindTexture(gogl.TEXTURE_2D, 0)
}
